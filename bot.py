...
def _build_cmd(inp, out, vc, res, fps, ac):
    ...
    if res["scale"]:
        if res.get("aspect") == "keep":
            # রেশিও ঠিক রেখে জাস্ট রেজুলেশন বাড়াবে/কমাবে
            vf.append(f"scale={res['scale']}:flags=lanczos")
        else:
            # চ্যাপ্টা করে হলেও নির্দিষ্ট রেজুলেশনে বসাবে
...
async def ytdlp_download_video(url: str, quality_height: int, out_path: str) -> str:
    """yt-dlp দিয়ে YouTube ভিডিও ডাউনলোড করে। Bot bypass arg অ্যাড করা হয়েছে।"""
    ...
    cmd = [
        ...
        "--no-playlist",
        "--no-progress",
        "--extractor-args", "youtube:player_client=android", # Bot Bypass
